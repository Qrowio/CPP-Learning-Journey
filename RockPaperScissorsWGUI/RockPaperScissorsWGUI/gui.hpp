...
public:
	...
	static void Panel(float xPosition,float yPosition, float pWidth,float pHeight,sf::Color pColor)
	{
		...
		if (!Init)
		{
			...
			std::cout << "[PNL] Appended " << Panel << " to m_Renderables!" << std::endl;
		}
	}
	...
	static bool Button(float xPosition, float yPosition, float bWidth, float bHeight, sf::Color bColor, std::string bText, sf::Color bTextColor, bool bBold = false)
	{
		...
		//std::cout << "[BTN] Appended " << Button << " and " << ButtonText << " to m_Renderables!" << std::endl;


		return m_Event->type == sf::Event::MouseButtonReleased && m_Event->mouseButton.button == sf::Mouse::Left && Button->getGlobalBounds().contains(m_Window->mapPixelToCoords(sf::Mouse::getPosition(*m_Window)));
	}

	static void TextPanel(float xPosition, float yPosition, float pWidth, float pHeight, sf::Color pColor, std::string pText, sf::Color pTextColor)
	{
		static bool Init = false;
		if (!Init)
		{
			...
			m_Renderables.push_back(*ButtonText);
			//std::cout << "[TEXT_PNL] Appended " << Panel << " to m_Renderables!" << std::endl;
		}
	}
